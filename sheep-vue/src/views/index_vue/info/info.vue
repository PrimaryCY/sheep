...
<script>
	...
	import {upload,patch_user} from "../../../api"
	...
	export default {
		...
		data(){
			return {
				...
				edit: {
					portrait: this.$store.state.user.portrait,
		...
		methods:{
			...
			modify_info(){
				...
				this.$refs['rule_edit'].validate(async (valid) => {
					...
					let res = await patch_user(this.edit)
					if(res.code!==2000){
						this.$message(res.msg)
						loading.close()
						return false
					}
					this.$store.dispatch('modify_userinfo', res.data)
			...
			async upload_portrait(){
				...
				let portrait_res = await upload(this.uploadData)
				if(portrait_res.code!==2000){
					this.$message(portrait_res.msg)
					loading.close()
					return
				}
				this.edit.portrait=portrait_res.data.url
				let user_res = await patch_user(this.edit)
				if(user_res.code!==2000){
					this.$message(user_res.msg)
					loading.close()
					return
				}
				this.uploadData.flag=!this.uploadData.flag
				this.$store.dispatch('modify_userinfo',user_res.data)
				this.$message.success('修改成功!')
				loading.close()
			},
